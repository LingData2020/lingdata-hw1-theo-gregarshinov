...
Answer: $\frac{5}{20}$, because there are only five values, that exceed $5$ and there are $20$ values in the sample.

b. Indicate the interval where the median of this sample can lie. Explain your answer. \\

Answer: For instance, the meaon on interval $[-15, 0]$ will definately lie, because the values are too unevenly distriputed. \\

c. How the histogram will change if we add an element $7$ to the sample? Explain your answer. \\
Answer: The bar $[5, 10]$ will grow by one point.
...
Answer: The sample number $1$ has a larger variavce, because its values vary on a larger interval ($[350, 650]$ vs. $[450, 50]$)
...
Which of the samples has a larger variance? Explain your answer. \\
Answer: The sample number $1$ has a larger variavce, because its values vary on a larger interval ($approximately [-8, 17]$ vs. $[8, 12]$)
...
According to the bar chart, the number of students, who made more than 35 mistakes is way less than the number of those who did not.
...
```
Explain your answer below: \\
The sum of frequencies on $(10, \infty)$ is bigger that on $[0, 10]$
...
```
Explain your answer below: \\
Because, if all the values in the sample were lain out in a sorted list, the frequencies around $20$ would make the most of the sample and occupy the center of the list.
...
```
Explain your answer below: \\
To explain this, the binomial distribution has to be used: \\

$B(N = 1000000, p=0) = {\binom{n}{k}}p^{k}(1-p)^{n-k}$ \\
$ \sum _{i=1}^{1000000}{1000000 \choose i}0^{i}(1-0)^{1000000-i} = 0 $ \\

this result (0 < 5%) indicates that we have evidence that is significant enough to reject the null hypothesis that the p = 0.
...
```
H0: The pobabilty of verbs being in the rhythming position is the same as being in in non-rhythming position, and equals 0.17
H1: The pobabilty of verbs staying in the rhythming position differs from the  probability of being in non-rhythming position.
```
...
df_1920 <- df[df$Decade == "1920s",]
verbs <- df_1920[df_1920$UPoS == "VERB",]
len_sample <- nrow(df_1920)
num_verbs <- nrow(verbs)
sprintf("Number of verbs used in 1920s: %s", num_verbs)
sprintf("The length of 1920s sample: %s", len_sample)
```
...
```{r}
binom.test(num_verbs, len_sample, p=0.17)
```
...
```
As the p-valueis less then 5%, we can conclude, that the null-hypothesis is rejectable.
```
...
```{r}
df_1820 <- df[df$Decade == "1820s",]
verbs <- df_1920[df_1920$UPoS == "VERB",]
len_sample <- nrow(df_1820)
num_verbs <- nrow(verbs)
sprintf("Number of verbs used in 1820s: %s", num_verbs)
sprintf("The length of 1920s sample: %s", len_sample)
binom.test(num_verbs, len_sample, p=0.17)
```
...
```
According to the p-values of samples from 1820s and 1920s, there is insufficient evidence to infer that verbs are not used in the rhyming position more often or less often than expected.
```
...
```
H0: the population mean of vowel duration in speaker `shg05` equals 73 (ms)
H1: the population mean of vowel duration in speaker `shg05` is greater or less than 73 (ms)
```

### 8.2
Perform a one-sample t-test. 
```{r}
df_islandic <- read.csv("https://raw.githubusercontent.com/LingData2019/LingData2020/master/data/icelandic.csv")
sample_shg05 <- df_islandic[df_islandic$speaker == "shg05",]
sample_mean <- mean(df_islandic$vowel.dur)
t.test(sample_shg05$vowel.dur, mu = sample_mean) # mu is a population mean
```

### 8.3
Interpret results. 
```
The p-value is way less than 0.05, hence the population mean of vowel duration in speaker `shg05` differs from 73.
```
s
